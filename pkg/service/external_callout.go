...
import (
	...
	"github.com/srinandan/apigee-opa-callout/pkg/apigee"
...
func (c *ExternalCalloutServiceServer) ProcessMessage(ctx context.Context, req *apigee.MessageContext) (*apigee.MessageContext, error) {
	...
	if err = json.Unmarshal([]byte(req.Request.Content), &regoInput); err != nil {
		...
		req.Request = nil
		req.AdditionalFlowVariables["opa-result"] = setBoolFlowVariable(false)
		response.Content = getErrorMessage(err.Error())
	} else {
		req.Request = nil
		...
		if err != nil {
			...
			req.AdditionalFlowVariables["opa-result"] = setBoolFlowVariable(rs[0].Expressions[0].Value.(bool))
